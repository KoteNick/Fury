...
void main() {
	...
	float distanceToCam = length(u_ViewPos.xyz - pos);

	float fade = smoothstep(20.0, 150.0, distanceToCam);
	
	vec3 true_derivative = vec3(-n.y / _Scale, 1.0, -n.z / _Scale);
	
	vec3 blended_derivative = mix(true_derivative, vec3(0.0, 1.0, 0.0), fade * 0.8);
